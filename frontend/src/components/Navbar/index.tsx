...
export const Template: React.FC = (props) => {
  ...
  const defaultIndex = children.find((child) => child.type === Links)?.props.children?.map((child:any) => child.props?.href).findIndex((child:string) => child===history.location.pathname);
  return (
    <Flex
      py={4}
      px={{ base: 4, md: 6, lg: 8 }}
    ...
    >
      ...
      <HStack display={{ base: "none", md: "flex" }} marginStart={4}>
        <Tabs colorScheme="blue" variant="unstyled" isFitted defaultIndex={defaultIndex}>
          <TabList>
      ...
      <HStack display={{ base: "none", md: "flex" }} spacing={3}>
        {children.find((child) => child.type === Settings)?.props.children}
      </HStack>
...
export const Navbar = Object.assign(Template, { Brand, Links, Settings });
